...
from grouping import remove_outliers, helper_object_points, max_min, max_min_3d
...
def input_3dnn(object_points, x_range, y_range, z_range, grid_size=1, img_length=16, img_height=16):
    """Function to create the input to the cnn function, need to see the time and optimize it later"""
    # Need to check the time
    # start_time = datetime.now()
    ...
    gridx = np.linspace(x_range[0], x_range[1], n_cols + 1)
    gridy = np.linspace(y_range[0], y_range[1], n_rows + 1)
    gridz = np.linspace(z_range[0], z_range[1], n_rows + 1)
...
def prepare_and_save_input(
    ...
    x_max, x_min, y_max, y_min, z_max, z_min = max_min_3d(new_objects, img_length=16, img_height=16)
...
def optimize(num_iterations, train_batch_size, input_train, output_train, session, x, y_true, optimizer, accuracy):
    ...
    # Batching the queue 
    x_batch2, y_true_batch2 = tf.train.batch([sliced_x, sliced_y], batch_size=train_batch_size,
...
def data_prep(save_here):
    ...
    list_of_object_choice = list(range(28))
