...
model <-c('2-layerCNN \n with single view input', '2-layerCNN \n with DBscan', '4-layerCNN \n with DBscan', '4-layerCNN \n with DBscan&Projection')
accuracy <-c(13.35, 39.79, 45.12, 47.06)
time<-c(578, 546, 556, 462)
...
pdf("evaluation2.pdf", height=2.5, width=8,family="Helvetica");

model_range<-c(1,2,3,4)

plot_colors <- c("blue","green","red","darkblue","grey90");

par(mfrow=c(1,5),oma=c(0,0,0,0),mar=c(5.5,3.5,0.5,0.5))
...
#axis(side=1, at=seq(1,4),labels=model , lwd.ticks=1,cex.axis=0.7,las=1,las=1)
...
#time
barplot(time,type = "l", col = plot_colors[5], xlab = "", ylab = "", main = "",lwd=2,pch=15,ylim=c(0,600),xaxs="i", yaxs="i",axes = FALSE);
...
title(ylab="Time (Second/Scene)", line=2, cex.lab=0.8, family="Helvetica")
abline(h=0,col="black",lwd=2)
